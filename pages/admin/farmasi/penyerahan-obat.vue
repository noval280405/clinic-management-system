<template>
  ...
  <v-row align="center">
    <v-col cols="9">
      <v-breadcrumbs>
        <v-breadcrumbs-item>
          <span class="font-weight-medium text-h5">Penyerahan Obat </span>
        </v-breadcrumbs-item>
      </v-breadcrumbs>
    </v-col>
...
onMounted(async () => {
  ...
  await resepStore.tarikDataResepObatByStatus("Diproses");
});
...
const listPoli = computed(() => {
  const poliSet = new Set(
    resepStore.getDataResepObat.map((i: any) => i.nama_poli)
  );
  return ["Semua", ...Array.from(poliSet)];
});
...
const filteredData = computed(() => {
  return resepStore.getDataResepObat.filter((item: any) => {
    ...
    return matchStatus && matchPoli && matchTanggal;
  });
});
